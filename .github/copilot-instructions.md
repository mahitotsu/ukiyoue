...
**同じ種類のドキュメントは厳密に同じ体裁で作成すること。**

これは非常に重要なルールです。新規ADRを作成する際は：

1. **必ず既存のADRを参照**してフォーマットを確認する
2. **完全に同一の構造**を維持する（セクション、見出しレベル、箇条書きスタイル等）
3. **以下の点を厳守**：
...
4. **作成後、必ず既存ADRと比較検証**する
5. フォーマットエラーがあれば即座に修正する

**教訓**: ADR-010/011作成時、度重なる修正が必要となった。最初から既存フォーマットを厳密に踏襲すれば、このような手戻りは発生しない。
