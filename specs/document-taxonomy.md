...
Project Charter
  ...
  └─→ Business Requirements（ユーザーストーリー含む）
        ├─→ Use Case Specification（ユーザーストーリーを詳細化）
        │     ├─→ Functional Requirements（ユースケースを機能仕様化）
        │     │     ├─→ UI/UX Specification
        │     │     └─→ Test Specification
        │     └─→ UI/UX Specification
...
**抽象度**: Business Requirements（WHY/WHO） > Use Case（WHAT） > Functional（HOW）
