...
このドキュメントは、Ukiyoue フレームワークで扱う45種類の成果物タイプの詳細定義を提供します。
...
- **主な内容**:
  ...
  - **ハイレベルマイルストーン**: 主要な節目（概算、経営判断用、Roadmapで詳細化）
  - **予算・リソース制約**: 投資規模、人員制約
...
- **識別子**: `PM-ROADMAP`
- **目的**: プロジェクトの時間軸とマイルストーンを管理（Project Charterのハイレベルマイルストーンを詳細化・追跡）
- **主な内容**:
  - **マイルストーン**: 主要なリリースポイント、フェーズゲート（具体的な日付・成果物）
  - **主要成果物と期限**: いつ何を完成させるか
  - **依存関係**: 前提条件、ブロッカー
  - **リソース計画**: 各フェーズの人員配置
- **入力**: Project Charter（プロジェクトスコープと目標）、**フィードバックループ**: Test Results（品質状況に基づくスケジュール調整）
...
- **入力**: Project Charter（初期リスクと前提条件）、**以降はプロジェクト全段階から継続的に追加**（Roadmap、Business Goal、設計、実装、テスト各段階での新規リスク識別）
...
- **主な内容**:
  ...
  - **関連リスク**: 目標達成を妨げるリスクへの参照（Risk Registerで管理）
- **入力**: Project Charter（ハイレベルなビジネス目標）
...
- **識別子**: `BIZ-DICT`
- **目的**: プロジェクト固有の用語とその属性レベルの詳細を定義し、チーム内の共通言語を確立し、成果物間での用語整合性を保証
- **主な内容**:
  - **用語定義**: ビジネス用語の意味と説明
  - **ドメイン分類**: business（ビジネス）/ system（システム）/ analytics（分析）/ infrastructure（インフラ）
  - **レイヤー分類**: conceptual（概念）/ logical（論理）/ physical（物理）
  - **データ型とフォーマット**: 技術的な型、表現形式、正規表現
  - **制約**: 必須/任意、一意性、最小/最大値、デフォルト値
  - **バリデーションルール**: 許容値、パターンマッチング（用語単体の制約）
  - **同義語**: 正規用語に対する別名、言語別の表記
  - **非推奨フラグ**: 用語の廃止・置き換え情報
- **入力**: なし（横断的成果物、プロジェクト全段階から継続的に追加）
- **読者**: 全員（開発者、テスター、BA、ステークホルダー、データカタログ管理者）
- **更新頻度**: プロジェクト初期に作成、新しい用語が登場する度に追加、仕様変更時、用語の非推奨化時
- **特徴**: **横断的管理文書**（プロジェクト全体の用語を一元管理、Risk RegisterやADRと同様にライフサイクル全体で更新される成果物、ビジネス用語を含むためLayer 1に配置）
- **活用方法**:
  - **用語参照**: Conceptual Data Model、Logical Data Model、API Architecture、Physical Data Model が `termReference` フィールドで用語を参照
  - **自動検証**: Reference Validator により、参照先の存在、型整合性、制約整合性を自動検証
  - **データカタログ統合**: RDF エクスポートで Collibra、Atlan、DataHub 等のデータカタログに登録可能（ADR-009）
...
- **入力**: Data Dictionary（エンティティとして扱う用語を特定）、Use Case（エンティティ間の相互作用）
...
- **入力**: Non-Functional Requirements（制約と品質要件）、**以降はアーキテクチャ設計全段階から継続的に追加**（Runtime Architecture、Infrastructure Architecture、Security Architecture等での技術決定）
...
- **入力**: Functional Requirements（画面で実現する機能）、Logical Data Model（画面に表示するデータ）
...
- **識別子**: `ARCH-API`
- **目的**: API設計方針とアーキテクチャパターンを定義（Layer 4のAPI Specificationへの設計指針）
- **主な内容**:
  - **API設計方針**: RESTful vs GraphQL vs gRPC、設計哲学と選定理由
  - **リソース設計原則**: URL構造、HTTPメソッド使用方針、リソース命名規則
  - **バージョニング戦略**: URLバージョニング vs ヘッダーバージョニング、互換性維持方針
  - **認証・認可パターン**: OAuth2フロー、JWT利用方針、スコープ設計、API Key管理
  - **エラーハンドリング方針**: エラーコード体系、エラーレスポンス構造、HTTPステータスコード使用方針
  - **ページネーション/フィルタリング**: クエリパラメータ設計、カーソルベース vs オフセットベース
  - **レート制限設計**: スロットリング戦略、クォータ設計、429エラー処理
  - **API ゲートウェイ戦略**: ルーティング、変換、集約、セキュリティポリシー
  - **ドキュメンテーション方針**: OpenAPI/Swagger生成方針、サンプルコード提供
- **入力**: Runtime Architecture（システムアーキテクチャとコンポーネント間通信）、Logical Data Model（APIで扱うデータ構造の基礎）、Security Architecture（API認証・認可方式）
- **読者**: APIアーキテクト、バックエンド開発者、フロントエンド開発者、API利用者
- **更新頻度**: 設計段階で作成、API設計方針変更時、アーキテクチャパターン追加時
- **特徴**: **API設計指針文書**（Layer 4のAPI Specification作成時の指針、技術選定とパターン定義、実装詳細はLayer 4で定義）
...
- **入力**: Functional Requirements（機能実装）、Implementation Guide（実装規約）、UI/UX Specification（画面実装）、Physical Data Model（データアクセス実装）
- **読者**: 開発者、保守担当者、コードレビュアー
- **更新頻度**: 実装段階で作成、継続的に更新（毎日）
- **特徴**: **実装コード**（最も多くの設計情報を統合する成果物、品質はTest Resultsで検証）

#### API Specification（API 仕様書）

- **識別子**: `IMPL-API`
- **目的**: 実装可能な詳細API仕様を機械可読形式で定義（OpenAPI/Swagger）
- **主な内容**:
  - **エンドポイント定義**: URL、HTTPメソッド、operationId、概要
  - **リクエストスキーマ**: パラメータ（path/query/header/body）、データ型、必須/任意、バリデーションルール
  - **レスポンススキーマ**: ステータスコード別のレスポンスボディ、ヘッダー、データ型
  - **認証・認可**: securitySchemes（API Key、OAuth2、JWT等）、スコープ定義
  - **エラーレスポンス**: エラーコード、エラーメッセージフォーマット、詳細説明
  - **サンプルリクエスト/レスポンス**: 具体例、cURLコマンド
  - **データモデル定義**: components/schemas（再利用可能なスキーマ定義）
  - **用語参照**: `termReference`フィールドでData Dictionaryの用語を参照（パラメータ、レスポンスフィールド）
- **入力**: API Architecture（API設計方針・パターン）、Runtime Architecture（API設計方針）、Logical Data Model（APIで扱うデータ構造）、Data Dictionary（API仕様で使用する用語の定義）
- **読者**: API開発者、フロントエンド開発者、API利用者（内部/外部）、自動生成ツール（コードジェネレータ、モックサーバー）
- **更新頻度**: API実装段階で作成、API変更時、バージョンアップ時
- **特徴**: **機械可読なインターフェース契約**（OpenAPI 3.0/Swagger形式、コード生成・モックサーバー生成・自動テスト生成が可能、ADR-005に基づきLayer 4に配置）
...
- **識別子**: `OPS-RUNBOOK`
- **目的**: 障害対応と問題診断の統合手順書（迅速な復旧と運用効率化）
- **主な内容**:
  - **インシデント対応プロセス**:
  ...
  - **トラブルシューティング手順**:
    - **よくある問題**: 頻発する問題、既知の問題のFAQ
    - **症状と原因**: 症状から原因を特定する診断フロー
    - **診断方法**: ログ確認、メトリクス確認、コマンド実行
    - **復旧手順**: 一時復旧（Workaround）、恒久対応のステップバイステップ手順
    - **エスカレーション判断**: どの段階で開発チームにエスカレートするか
- **入力**: Observability Architecture（障害検知の仕組み）、Operations Manual（通常運用との違い）、Source Code Documentation（実装詳細の理解）
- **読者**: 運用担当者、SRE、サポート担当者、オンコール担当者、開発者、新規運用メンバー
- **更新頻度**: 運用開始時に作成、インシデント経験時に継続的に更新（FAQ追加、診断フロー改善）
- **特徴**: **統合障害対応手順書**（インシデント対応プロセスと技術的診断方法を統合、運用経験を蓄積するナレッジベース）
